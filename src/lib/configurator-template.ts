// IMMUTABILITY RULE: Item IDs are the only join key between this hardcoded template
// and stored state. Once merged to main, NEVER rename an ID. If an item needs to be
// replaced, add a new ID and mark the old one with // @deprecated - retained for historical join.
...
export interface ConfiguratorItemCondition {
  ...
  tab?: string;         // must be in enabledTabs[]
  featureToggle?: string; // must be true in featureToggles
}
...
export interface ConfiguratorTemplateItem {
  ...
  contextFields?: Array<{
    label: string;
    path: string; // dot-notation into checklist data, e.g. "companyInfo.companyWebsiteUrl"
  }>;
  requires?: ConfiguratorItemCondition; // omit = always included
}
...
export interface ConfiguratorItemState {
  ...
  notes: string | null;
  createdAt: string;   // ISO — set on first generation, never changes
  updatedAt: string | null; // ISO — null until status or notes actually change
  updatedBy: string | null; // admin user email of last editor
  archived: boolean;   // true = item no longer applicable under current config
}

export interface ConfiguratorChecklistBlob {
  items: ConfiguratorItemState[];
  snapshotItemIds: string[];  // item IDs applicable at last generation/refresh — drives progress denominator
  generatedAt: string;
  lastSnapshotAt: string;     // last time the snapshot was refreshed
}
...
export const CONFIGURATOR_TEMPLATE: ConfiguratorTemplateItem[] = [
  // Company Settings
  {
    ...
    contextFields: [
      ...
      { label: 'Address', path: 'companyInfo.companyAddress' },
      { label: 'Website', path: 'companyInfo.companyWebsiteUrl' },
      { label: 'Privacy policy URL', path: 'companyInfo.privacyPolicyUrl' },
      { label: 'Primary color', path: 'companyInfo.companyColor' },
    ],
  },
  {
    id: 'company-logos',
  ...
  // Facebook
  {
    ...
    contextFields: [
      { label: 'Facebook Page', path: 'companyInfo.fbPageName' },
      { label: 'Page URL', path: 'companyInfo.fbPageUrl' },
    ],
  },
  {
  ...
  // Instagram
  {
    ...
    contextFields: [
      { label: 'Instagram account', path: 'instagram.instagramAccount' },
    ],
  },
  {
